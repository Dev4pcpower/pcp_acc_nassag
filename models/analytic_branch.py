...
class BranchAnalyticDistribution(models.Model):
    ...
    account_id = fields.Many2one('branch.analytic.branch', string='Analytic branch', required=True)
...
class BranchAnalyticTag(models.Model):
    ...
    active = fields.Boolean(default=True, help="Set active to false to hide the branch Analytic Tag without removing it.")
...
class BranchAnalyticLine(models.Model):
    ...
    account_id = fields.Many2one('branch.analytic.branch', 'Analytic Branch', required=True, ondelete='restrict', index=True, check_company=True)
    partner_id = fields.Many2one('res.partner', string='Partner', check_company=True)
    user_id = fields.Many2one('res.users', string='User', default=_default_user)
    tag_ids = fields.Many2many('branch.analytic.tag', 'branch_analytic_line_tag_rel', 'line_id', 'tag_id', string='Tags', copy=True, check_company=True)
    ...
    group_id = fields.Many2one('branch.analytic.group', related='account_id.group_id', store=True, readonly=True, compute_sudo=True)
...
class BranchMoveLine(models.Model):
    ...
    branch_analytic_tag_ids = fields.Many2many('branch.analytic.tag', string='Analytic Branch Tags',
        compute="_compute_branch_analytic_tag_ids", store=True, readonly=False, check_company=True, copy=True)
...
class BranchAnalyticGroup(models.Model):
    ...
    parent_id = fields.Many2one('branch.analytic.group', string="Parent", ondelete='cascade', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
...
class BrachAnalyticBranch(models.Model):
    ...
    def read_group(self, domain, fields, groupby, offset=0, limit=None, orderby=False, lazy=True):
        ...
        """
        res = super(BrachAnalyticBranch, self).read_group(domain, fields, groupby, offset=offset, limit=limit, orderby=orderby, lazy=lazy)
        accounts = self.env['branch.analytic.branch']
        for line in res:
            if '__domain' in line:
