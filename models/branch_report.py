...
class BranchReportManager(models.Model):
    _name = 'branch.report.manager'
    ...
    summary = fields.Char()
    footnotes_ids = fields.One2many('branch.report.footnote', 'manager_id')
...
class AccountReport(models.AbstractModel):
    ...
    def _init_filter_analytic(self, options, previous_options=None):
        ...
        enable_analytic_accounts = self.user_has_groups('pcp_acc_nassag.group_analytic_branching')
        enable_analytic_tags = self.user_has_groups('pcp_acc_nassag.group_analytic_branching')
        ...
        if enable_analytic_tags:
            ...
            selected_analytic_tags = self.env['branch.analytic.tag'].search([('id', 'in', analytic_tag_ids)])
    ...
    def _get_options_analytic_domain(self, options):
        ...
        if options.get('analytic_tags'):
            ...
            domain.append(('branch_analytic_tag_ids', 'in', analytic_tag_ids))
        return domain
    ...
    def _get_report_manager(self, options):
        ...
        existing_manager = self.env['branch.report.manager'].search(domain, limit=1)
        if not existing_manager:
            existing_manager = self.env['branch.report.manager'].create({
